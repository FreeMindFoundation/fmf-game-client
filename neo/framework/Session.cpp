...
void TestGUI_f( const idCmdArgs &args ) {
	const char *p = ( args.Argc() > 1 ) ? args.Argv( 1 ) : "";
	sessLocal.TestGUI1( p, true );
}

void idSessionLocal::TestGUI1( const char *p, bool extended ) {
	idUserInterface *tgui;
	tgui = uiManager->FindGui( "guis/mpmain.gui", true, false, true );

	//SetGUI( tgui, NULL );
	tgui->SetStateBool( "gameDraw", true );
	tgui->Activate( true, com_frameTime );
}
...
void idSessionLocal::Draw() {
	...
	//if( guiTakeNotes != NULL )
	//	guiTakeNotes->Redraw( com_frameTime );
}
...
void idSessionLocal::Init() {
	...
	cmdSystem->AddCommand( "testgui", TestGUI_f, CMD_FL_SYSTEM, "test gui" );
	...
	guiMainMenu = uiManager->FindGui( "guis/dog/mainmenu.gui", true, false, true );
#else
